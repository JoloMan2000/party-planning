"""Statische ISO-3166-1-alpha-2-Länderliste (Geo-Kultur-Spec §13: UI-Wiring).

Rein für die Admin-Override-Dropdown in ``Party Planning.py`` gedacht (voller
Wertebereich, nicht nur die kuratierten Länder aus ``culture.py`` - auch ein
Land ohne eigenes ``CultureProfile`` soll als expliziter Override wählbar
sein und fällt dann auf das neutrale ``CultureProfile`` zurück, siehe
``party_context.culture.get_culture_profile``). Enthält bewusst KEINE Logik,
nur die reine Stammdaten-Zuordnung Code -> englischer Name."""
...
if __name__ == "__main__":
    ...
    assert all(len(code) == 2 and code.isupper() for code in ISO_COUNTRIES)
    assert len(ISO_COUNTRIES) == len(set(ISO_COUNTRIES.values()))
    for expected in ("DE", "IN", "PE", "DK", "JP", "US"):
        assert expected in ISO_COUNTRIES, expected
    print(f"party_context/countries.py sanity check OK ({len(ISO_COUNTRIES)} countries).")
