...
0b/0c), party-gescoped seit dem Multi-Tenant-Pivot Phase 4. Nutzt eine
temporäre sqlite-DB (``tmp_path``) für die Persistenz-Tests und den ECHTEN
Katalog (kein Mocking, siehe conftest.py).
...
def test_get_derived_party_context_guest_count_null_wird_zu_eins(tmp_path):
    ...
    derived = get_derived_party_context(db_path, PARTY_ID, settings, guest_count=0)
...
def test_maybe_freeze_ohne_party_date_gibt_false(tmp_path):
    ...
    settings = event_theme.get_party_settings(db_path, PARTY_ID)
    assert maybe_freeze_and_reset_party(db_path, PARTY_ID, HOST_USER_ID, settings, catalog=None) is False
...
def test_maybe_freeze_bei_zukuenftigem_datum_gibt_false(tmp_path, catalog):
    ...
    settings = event_theme.get_party_settings(db_path, PARTY_ID)
    assert maybe_freeze_and_reset_party(db_path, PARTY_ID, HOST_USER_ID, settings, catalog) is False
...
def test_maybe_freeze_bei_vergangenem_datum_mit_responses_reset_und_lernt(tmp_path, catalog):
    ...
    settings = event_theme.get_party_settings(db_path, PARTY_ID)
    result = maybe_freeze_and_reset_party(db_path, PARTY_ID, HOST_USER_ID, settings, catalog)

    assert result is True
    assert response_storage.load_responses(db_path, PARTY_ID) == []  # responses geleert
    # party_settings selbst bleibt als wiederverwendbare Vorlage erhalten.
    assert event_theme.get_party_settings(db_path, PARTY_ID)["party_date"] == "2020-01-01"


def test_maybe_freeze_loescht_nur_responses_der_eigenen_party(tmp_path, catalog):
    """Regressionstest für den kritischen Multi-Tenant-Bug: ``DELETE FROM
    responses`` ohne WHERE-Klausel hätte alle Partys geleert."""
    db_path = tmp_path / "context.db"
    _init_all(db_path)

    other_party_id = "other-party"
    recipe_id = next(iter(catalog.recipes))
    response_storage.save_response(
        db_path, other_party_id, name="Ben", start_time="18:00",
        drinks=[recipe_id], drinks_freetext="", food=[], food_freetext="", songs=[],
    )
    ...
    response_storage.save_response(
        db_path, PARTY_ID, name="Max", start_time="19:00",
        drinks=[recipe_id], drinks_freetext="", food=[], food_freetext="", songs=[],
    )
    ...
    assert result is True
    assert response_storage.load_responses(db_path, PARTY_ID) == []
    assert len(response_storage.load_responses(db_path, other_party_id)) == 1
